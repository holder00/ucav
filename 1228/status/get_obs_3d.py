...
def get_state_mrm(env,mrm,is_launched):
    ...
    if is_launched:
        ...
        uav_state["self_pos_x"] = np.array([lat_norm(mrm.pos[0]/env.WINDOW_SIZE_lat)])
        uav_state["self_pos_y"] = np.array([harf(mrm.pos[1]/env.WINDOW_SIZE_lon)])
        uav_state["self_pos_z"] = np.array([harf(mrm.pos[2]/env.WINDOW_SIZE_alt)])
    ...
    for i in range(env.blue_num):
        ...
        for j in range(env.blue_num):
            if i == j:
                ...
                uav_id = uav_id +1
        # obs['blue_' + str(i)]["reds"] = get_state_red(env,nearest_id,nearest_distance)
        ...
        for j in range(env.red_num):
            if nearest_id[i] == 0:
                ...
                ids = 1
            nearest_distances = nearest_dist[i,j]
            op_psi = calc_each_ops_psi(env.blue[i],env.red[ids])
            op_gam = calc_each_ops_gam(env.blue[i],env.red[ids])
            # print(ids,nearest_id)
            obs['blue_' + str(i)]["reds"]["red_" + str(j)] = get_state_red(env,ids,nearest_distances,op_psi,op_gam)
