...
def get_observation(env):
    ...
    observations = get_observation_6(env)  # 7ch
...
def get_observation_6(env):
    """
    Observation maps : 7ch including obstacles
    Normalized by current number of team mates based on observation_5
    """
    observation = {}
    ...
    for i in range(env.num_red):
        if env.red.alive[i]:
            ...
            # Obstacles map
            block_matrix = np.zeros((env.grid_size, env.grid_size))
            ...
            # Block position map
            for j in range(env.num_block):
                block_matrix[env.block.pos[j][0], env.block.pos[j][1]] = 1
            block_matrix = np.expand_dims(block_matrix, axis=2)

            obs = np.concatenate([my_matrix_pos, my_matrix,
                                  teammate_matrix_pos, teammate_matrix,
                                  adversarial_matrix_pos, adversarial_matrix,
                                  block_matrix], axis=-1)  # (10,10,7)
