...
class MyEnv(gym.Env):
    ...
    SPACE_Y = SPACE_X  # km
    SPACE_OFFSET = 50.  # km, negative size of battle space default
    # SPACE_OFFSET = 10.
    ...
    def set_initial_condition_fighter(self, fighter):
        r = 45 + 5 * np.random.rand()  # default
        # r = 90 + 10 * np.random.rand()
        ...
        heading_error = 20 * np.pi / 180 * (np.random.rand() - .5) * 2
        ...
        c = np.random.rand()
        if c < .5:
            fighter.x = 20 * np.random.rand()
            fighter.y = 80 * np.random.rand()
        else:
            fighter.x = 100 * np.random.rand()
            fighter.y = 80 + 20 * np.random.rand()

        r_vec = np.array([self.space_x - fighter.x, 0 - fighter.y])

        r = np.linalg.norm(r_vec, ord=2)
        heading = np.arcsin(-r_vec[1] / r) + np.pi / 2
        heading_error = 20 * np.pi / 180 * (np.random.rand() - 0.5) * 2

        fighter.heading = heading + heading_error
        fighter.heading_sin = np.sin(fighter.heading)
        fighter.heading_cos = np.cos(fighter.heading)
        """
