...
from settings.initial_settings_2 import *
...
model_path = PROJECT + '/checkpoints/' + TRIAL + '/checkpoint_000677/checkpoint-677'
...
if __name__ == '__main__':
    # framework = "tf2"
    # as_test = True
    test_id_list = [30, 32, 33, 34, 35, 40, 42, 43, 44, 45, 50, 52, 53, 54, 55]
    red_num_list = [1, 3, 5, 8, 10, 1, 3, 5, 8, 10, 1, 3, 5, 8, 10]
    blue_num_list = [5, 5, 5, 5, 5, 7, 7, 7, 7, 7, 10, 10, 10, 10, 10]

    if (len(test_id_list) != len(red_num_list)) or (len(test_id_list) != len(blue_num_list)):
        raise print('Size is wrong')

    for TEST_ID, num_red, num_blue in zip(test_id_list, red_num_list, blue_num_list):
        NUM_RED_MAX = num_red
        NUM_RED_MIN = num_red
        NUM_BLUE_MAX = num_blue
        NUM_BLUE_MIN = num_blue
