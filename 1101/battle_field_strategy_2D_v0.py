...
class BattleFieldStrategy(MultiAgentEnv):
    def __init__(self, config={}):
        ...
        self.num_block_max = NUM_BLOCK_MAX
        self.num_block_min = NUM_BLOCK_MIN
    ...
    def isno_block(self, x, y):
        """
        (x,y)にblockがあればFalse、無ければTrueをreturn
        """
        flag = True
        for z in self.block.pos:
            if (x == z[0]) and (y == z[1]):
                flag = False
        return flag
    ...
    def step(self, action_dict):
        ...
        for i in range(self.num_red):
            if self.red.alive[i]:
                ...
                if (action == 1) and (self.isno_block(self.red.pos[i][0] - 1, self.red.pos[i][1])):
                    self.red.pos[i][0] -= 1
                elif (action == 2) and (self.isno_block(self.red.pos[i][0] + 1, self.red.pos[i][1])):
                    self.red.pos[i][0] += 1
                elif (action == 3) and (self.isno_block(self.red.pos[i][0], self.red.pos[i][1] - 1)):
                    self.red.pos[i][1] -= 1
                elif (action == 4) and (self.isno_block(self.red.pos[i][0], self.red.pos[i][1] + 1)):
                    self.red.pos[i][1] += 1
