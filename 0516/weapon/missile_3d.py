...
class missile_3d(uav_3d):
    def __init__(self, parent):
        ...
        self.faction = "mrm"
        self.destruction_range = 300
        self.destruction_angle = 180
        self.tgt = parent.tgt
        self.mass = 200 #kg
        self.g = 9.80665 #m/ss
        self.Cd2 = 3.0132
    ...
    def update_status(self, sim_dt):
        if self.hitpoint > 0:
            ...
            if (self.hitpoint_ini -self.hitpoint) < 2: #2
                self.T = 6000*self.g
                self.mass = 200
                K = 0.01
            elif (self.hitpoint_ini -self.hitpoint) < 10: #10
                self.T = 920*self.g
                self.mass = 157
                K = 0.1
            else:
                self.T = 50*self.g
                self.mass = 130 #130
                K = 0.1+0.4*(1-(self.hitpoint/(self.hitpoint_ini-10))**2)
                
            err_gam = harf_angle(self.ops_gam() - self.gam)
            err_psi = harf_angle(self.ops_az() - self.psi)

            self.CD = self.CD0 + 0.05*0.1*self.mass/(self.rho*(self.V**2)*self.S)*(1+0.1*np.sin(np.abs(err_psi)))
            ...
            if (self.tgt_inrange() or self.parent.tgt_inrange()) and not self.tgt.hitpoint == 0:
                self.parent.has_active_missile = True
                self.gam = harf_angle(self.gam+ K*err_gam)
                self.psi = harf_angle(self.psi+ K*err_psi)
