...
class MyEnv(MultiAgentEnv):
    ...
    def reset(self):
        ...
        if  self.sim_num < self.cpu_num :
            self.sim_num = 0
        else:
            self.sim_num += self.sim_use_num
        self.reward_win = 0
        self.rewards_total = {}
        self.reward_total = 0
        self.sim_comp = False
        ...
        if not self.init_flag:
            ...
            # #exe起動待ち
            # time.sleep(0.5)
            subprocess.Popen(r"./UCAV_vec.exe /BLUE 2 /RED 0 /MEM "+str(self.sim_num+1))
            #exe起動待ち
            time.sleep(0.5)
            subprocess.Popen(r"./UCAV_vec.exe /BLUE 2 /RED 0 /MEM "+str(self.sim_num+2))
            #exe起動待ち
            time.sleep(0.5)

            #共有メモリに初期値設定
            # for i in range(self.blue_num):
            self.SetPyToSimData(self.blue,self.red,1,0,self.sim_num+1)
            self.SetPyToSimData(self.red,self.blue,1,0,self.sim_num+2)
            # for i in range(self.red_num):
    ...
    def step(self, action_dict):
        sim_num = copy.deepcopy(self.sim_num)
...
#python ⇒ uavsim
        ...
        # for i in range(self.blue_num):
        self.SetPyToSimData(self.blue,self.red,1,self.timer,sim_num+1)
        self.SetPyToSimData(self.red,self.blue,1,self.timer,sim_num+2)
        ...
        while not self.sim_comp:
            self.sim_time = 0
            # for i in range(self.blue_num):
            self.GetSimToPyData(self.blue,self.red,sim_num+1,"blue",i)
            self.GetSimToPyData(self.red,self.blue,sim_num+2,"red",i)
                
            # for i in range(self.red_num):
            #     self.GetSimToPyData(self.blue,self.red,i+self.blue_num+1,"red",i)
                
            if self.sim_time == (self.blue_num + self.red_num)*0+2:
                self.sim_comp = True
        else:
            self.sim_comp = False
...
#python ⇒ uavsim
        ...
        if is_touch or is_fin or is_fin_too_close or is_fin_splashed or self.timer >= self.time_limit:
            dones["__all__"] = True
            # for i in range(self.blue_num):
            self.SetPyToSimData(self.blue,self.red,1,-1,sim_num+1)
            self.SetPyToSimData(self.red,self.blue,1,-1,sim_num+2)
            # for i in range(self.red_num):
    ...
    def SetPyToSimData(self,blue,red,flag,stepcnt,memno):
        ...
        for i in range(len(blue)):
            BlueData[i] = [blue[i].pos[0], blue[i].pos[1], blue[i].pos[2], blue[i].V*0+250.0, np.rad2deg(blue[i].gam)*0.0, np.rad2deg(blue[i].psi)*0.0, blue[i].fire, blue[i].fire]
            BlueData[i][6] = 1
            BlueData[i][7] = 1
            RedData[i][6] = 1
            RedData[i][7] = 1
            # BlueData[i] = [copy.deepcopy(blue[i].pos[0]),
            #                copy.deepcopy(blue[i].pos[1]),
            #                copy.deepcopy(blue[i].pos[2]),
            #                250.0, 
            #                0.0,
            #                135.0,
            #                1,
        ...
        # if stepcnt > 0:
        #     for i in range(len(blue)):
        #         # BlueData[i][0] = copy.deepcopy(blue[i].pos_ref[0])
        #         # BlueData[i][1] = copy.deepcopy(blue[i].pos_ref[1])
        #         # BlueData[i][2] = copy.deepcopy(blue[i].pos_ref[2])
        #         # BlueData[i][3] = 250.0
        #         # BlueData[i][4] = 0.0
        #         # BlueData[i][5] = 135.0
        #         # if blue[i].fire:
        #         #     BlueData[i][6] = 1
        #         #     BlueData[i][7] = 1
        #         BlueData[i][3] = blue[i].V_ref*0+250.0
        #         BlueData[i][4] = np.rad2deg(blue[i].gam_ref)*0.0
        #         BlueData[i][5] = np.rad2deg(blue[i].psi_ref)*0.0
        #         if blue[i].fire:
        #             BlueData[i][6] = 1
        #             BlueData[i][7] = 1
        if blue[0].faction == "blue":
            self.BB1 = BlueData
        if blue[0].faction == "red":
            self.BB2 = BlueData
            # print(memno,BlueData)
        ...
        OutValue = pack('ddddddiiddddddiiddddddiiddddddiiii',
                        ...
                        BlueData[1][0], BlueData[1][1], BlueData[1][2], BlueData[1][3], BlueData[1][4], BlueData[1][5], BlueData[1][6],BlueData[0][7],
    ...
    def GetSimToPyData(self,blue,red,memno,faction,uav_id):
        ...
        if faction == "blue":
            for i in range(self.blue_num):
                self.blue[i].pos[0] = copy.deepcopy(InValue[6+i*9])
                self.blue[i].pos[1] = copy.deepcopy(InValue[7+i*9])
                self.blue[i].pos[2] = copy.deepcopy(InValue[8+i*9])
                self.blue[i].V = copy.deepcopy(InValue[0+i*9])
                self.blue[i].gam = np.deg2rad(copy.deepcopy(InValue[1+i*9]))
                self.blue[i].psi = np.deg2rad(copy.deepcopy(InValue[2+i*9]))
                #rad→°
                self.blue[uav_id].phi = copy.deepcopy(InValue[3+i*9])
                self.blue[uav_id].pitch = copy.deepcopy(InValue[4+i*9])
                self.blue[uav_id].yaw = copy.deepcopy(InValue[5+i*9])
        if faction == "red":
            for i in range(self.red_num):
                self.red[i].pos[0] = copy.deepcopy(InValue[6+i*9])
                self.red[i].pos[1] = copy.deepcopy(InValue[7+i*9])
                self.red[i].pos[2] = copy.deepcopy(InValue[8+i*9])
                self.red[i].V = copy.deepcopy(InValue[0+i*9])
                self.red[i].gam = np.deg2rad(copy.deepcopy(InValue[1+i*9]))
                self.red[i].psi = np.deg2rad(copy.deepcopy(InValue[2+i*9]))
                #rad→°
                self.red[i].phi = copy.deepcopy(InValue[3+i*9])
                self.red[i].pitch = copy.deepcopy(InValue[4+i*9])
                self.red[i].yaw = copy.deepcopy(InValue[5+i*9])
