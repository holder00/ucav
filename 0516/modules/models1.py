...
class MyConv2DModel_v0(TFModelV2):
    def __init__(self, obs_space, action_space, num_outputs, model_config, name):
        ...
        """ Policy network """
        layer_1 = Flatten(filters=64, kernel_size=3, strides=1, activation='relu', padding='valid') \
            (self.inputs)
        layer_2 = Flatten(filters=128, kernel_size=3, strides=2, activation='relu', padding='valid') \
            (layer_1)
        layer_3 = Flatten(filters=128, kernel_size=3, strides=2, activation='relu', padding='valid') \
            (layer_2)
...
class DenseNetModelLargeShare_2(TFModelV2):
    def __init__(self, obs_space, action_space, num_outputs, Model_config, name):
        super(DenseNetModelLargeShare_2, self).__init__(obs_space, action_space, num_outputs, Model_config, name)
        num_init_features = 32
        growth_factor = 32
        block_layers = [3, 2, 2, 1]
        compression_factor = 0.5
        ...
        # Define policy model
        z = self.pol_dense_block_4(x)
        z = self.pol_avgpool(z)
        pol_out = self.pol_fc(z)  # (None, 5)
...
class DenseNetModelLargeShare_3(TFModelV2):
    def __init__(self, obs_space, action_space, num_outputs, Model_config, name):
        super(DenseNetModelLargeShare_3, self).__init__(obs_space, action_space, num_outputs, Model_config, name)
        num_init_features = 32
        growth_factor = 32
        block_layers = [3, 2, 2, 1]
        compression_factor = 0.5
        ...
        # Transition Layer 3
        self.pol_num_channels = self.num_channels + growth_factor * block_layers[2]
        self.pol_num_channels *= compression_factor
        self.pol_transition_3 = TransitionLayer(out_channels=int(self.pol_num_channels))
        ...
        # Define policy model
        z = self.pol_dense_block_3(x)
        z = self.pol_transition_3(z)
        z = self.pol_dense_block_4(z)
        z = self.pol_avgpool(z)
        pol_out = self.pol_fc(z)  # (None, 5)
        ...
        # Transition Layer 3
        self.val_num_channels = self.num_channels + growth_factor * block_layers[2]
        self.val_num_channels *= compression_factor
        self.val_transition_3 = TransitionLayer(out_channels=int(self.val_num_channels))
