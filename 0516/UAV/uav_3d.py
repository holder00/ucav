...
class uav_3d:
    def __init__(self, lat_lim, lon_lim, safe_area, faction, uav_id, tgt):
        ...
        if self.faction == "blue":
            ...
            # self.pos = np.array([37.5*1000, 150*1000, 10*1000])
        ...
        self.pos_ref = copy.deepcopy(self.pos)
    ...
    def can_launch(self):
        ...
        launch = False
        # self.fire = False
        if self.fire and self.inrange and self.cool_down > self.cool_down_limit and self.mrm_num > 0 and np.linalg.norm(self.pos - self.tgt.pos) <= self.mrm_range:
            self.cool_down = 0
    ...
    def MWS(self, mrm):
        if self.faction == mrm.tgt.faction and self.id == mrm.tgt.id and mrm.hitpoint > 0 and self.hitpoint > 0 and mrm.inrange:
